...
func (a *MusicService) PlayerLoad(track format.MscData) error {
	return a.player.loadTrack(track)
}

// PlayerPlay 恢复播放
func (a *MusicService) PlayerPlay() {
	a.player.resume()
}

// PlayerPause 暂停播放
func (a *MusicService) PlayerPause() {
	a.player.pause()
}

// PlayerToggle 切换播放/暂停
func (a *MusicService) PlayerToggle() {
	a.player.toggle()
}

// PlayerSeek 跳转到指定秒数
func (a *MusicService) PlayerSeek(seconds float64) error {
	return a.player.seek(seconds)
}

// PlayerStop 停止并清空当前曲目
func (a *MusicService) PlayerStop() {
	a.player.stop()
}

// PlayerGetState 返回当前播放器状态快照
func (a *MusicService) PlayerGetState() PlayerState {
	return a.player.snapshot()
}

// PlayerTogglePlayMode 切换播放模式并返回新模式
func (a *MusicService) PlayerTogglePlayMode() string {
...
func (a *MusicService) PlayerRestart() error {
...
func (a *MusicService) PlayerGetEqBandCount() int {
	return EqBandCount
}
...
func (a *MusicService) PlayerGetSmartEQEnabled() bool {
	smartEQ := a.player.SmartEQ()
	if smartEQ == nil {
		return false
	}
...
func (a *MusicService) PlayerGetSmartEQIntensity() float64 {
	smartEQ := a.player.SmartEQ()
	if smartEQ == nil {
		return 0
	}
...
func (a *MusicService) QueueGetStatus() QueueStatus {
